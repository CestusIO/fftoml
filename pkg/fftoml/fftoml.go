...
// relevant flag name.
// allows skipping a tablelayer. so that toskip.keyname can fill flags named keyname
...
func (c ConfigFileParser) Parse(r io.Reader, set func(name, value string) error) error {
	...
	return parseTree(tree, "", c.delimiter, c.skipTable, set)
}
...
func WithTableSkip(d ...string) Option {
	return func(c *ConfigFileParser) {
		c.skipTable = strings.Join(d, c.delimiter)
	}
}

func parseTree(tree *toml.Tree, parent, delimiter string, tableskip string, set func(name, value string) error) error {
	for _, key := range tree.Keys() {
		...
		case *toml.Tree:
			if err := parseTree(t, name, delimiter, tableskip, set); err != nil {
				return err
			}
		case interface{}:
			...
			for _, value := range values {
				var skip string
				if len(tableskip) > 0 {
					skip = tableskip + delimiter
				}
				if err = set(strings.TrimPrefix(name, skip), value); err != nil {
					return err
				}
			}
		}
